...
/**
 * Struct to store relevant info about the edges
 */
...
/**
 * Class to handle the connections between edges for Kruskal's algo
 */
...
/**
 * Class that creates the Minimum Spanning Tree required for the solution
 */
...
class MinimumSpanningTree {
    ...
    public:
        ...
        int kruskal(std::vector<Edge> &list, DisjointSetUnion &dsu) {
...
int main() {
    InParser cin("zapada.in");
    OutParser cout("zapada.out");
    ...
    while (k--) {
        cin >> u;
        u--;
        dsu.unite(list[u].parent, list[u].child);
        ans += list[u].cost;
    }

    MinimumSpanningTree mst(n);
    cout << ans + mst.kruskal(list, dsu) << '\n';
    return 0;
}